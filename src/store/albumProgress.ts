...
const AUDIOBOOK_GENRES = new Set([
  ...
  'yuseisho',
  'audiobukku',
  // Story & Fiction sub-genres commonly used for audiobooks and audio plays
  'thriller',
  'psychothriller',
  'psycho thriller',
...
/**
 * Key indicator terms for matching combined sub-genres (e.g., "Hörbuch - Thriller",
 * "Audiobook / Fantasy", "Krimi-Hörspiel", "Sci-Fi Audio Drama", "Psychothriller").
 */
const AUDIOBOOK_INDICATORS = [
  'audiobook',
  'audiobooks',
  'audiobooki',
  'horbuch',
...
export function isAudiobookGenre(v: string | null | undefined): boolean {
  if (!v) return false;
  return genreParts(v).some(
    (part) => AUDIOBOOK_GENRES.has(part) || AUDIOBOOK_INDICATORS.some((ind) => part.includes(ind)),
  );
}
